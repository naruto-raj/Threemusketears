...
def story_gen(text_prompt, chain1, chain2):
    ...
    return base64_video_data, story

@app.route('/')
def index():
    # Initially, set the default video URL
    video_url = 'sample_video.mp4'
    return render_template('index.html', video_url=video_url, initial_text_content = "You are a hungry Knight entering the realm of dragons")
...
def submit():
    ...
    if request.method == 'POST':
        ...
        base64_video_data, story = story_gen(text_prompt, chain1, chain2)
        story = story.replace('\n', ' ').replace("'", "").replace('"', '')
        ...
        # print(video_url)
        return render_template('index.html', initial_text_content = story, base64_video_data=base64_video_data)
        # return render_template('index.html', initial_text_content = story, video_url = 'sample_video.mp4')
